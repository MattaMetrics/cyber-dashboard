...
export default function MasterAssessmentDirectory({ onSelectTrack, onNavigate }) {
  ...
  return (
    <div className="w-full h-[calc(100vh-3.5rem)] bg-[#030712] p-6 md:p-8 font-mono text-white text-left select-none flex flex-col overflow-hidden">
      ...
      <div className="border border-slate-900 rounded-lg p-6 mb-6 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-slate-950/20 shrink-0">
        <TitleWithCyberSphere size="md" className="flex-1 w-full lg:w-auto">
          <div>
        ...
        <div className="text-right flex items-center space-x-4 shrink-0 self-end lg:self-auto">
        ...
        {alphabetizedTracks.map((track) => (
          ...
          >
            <div className="flex items-center gap-4 min-w-0 flex-1">
              ...
              <MethodologyCyberSphere size="xs" className="hidden sm:flex" />
              <span className="text-slate-600 text-[9px] uppercase tracking-widest shrink-0 hidden md:inline">
                [ {track.category} ]
              </span>
            </div>
